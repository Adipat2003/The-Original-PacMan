...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	uint8_t buffer[2] = {reg + 2, val};

	if (XSpi_Transfer(&SpiInstance, buffer, NULL, 2))
		xil_printf ("Failed Single Host Register Write!\r\n");
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    if(XSpi_Transfer(&SpiInstance, buffer, NULL, nbytes + 1))
		xil_printf ("Failed Multiple-Byte Write!\r\n");
...
BYTE MAXreg_rd(BYTE reg) {
	...
	if(XSpi_Transfer(&SpiInstance, buffer, buffer, 2))
		xil_printf ("Failed Single Host Register Read!\r\n");
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if (XSpi_Transfer(&SpiInstance, buffer, buffer, nbytes + 1))
	    	xil_printf ("Failed Multiple-bytes Register Read !\r\n");
